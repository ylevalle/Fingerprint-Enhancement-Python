...
def ridge_filter(im: NumpyArray, orient: NumpyArray, freq: NumpyArray,
                 kx: float, ky: float):
    ...
    x, y = np.meshgrid(np.linspace(-sze, sze, int(2 * sze + 1)),
                       np.linspace(-sze, sze, int(2 * sze + 1)))

    reffilter = np.exp(
        -(np.power(x, 2) / (sigmax * sigmax)
        +(np.power(y, 2)) / (sigmay * sigmay))
    ) * np.cos(2*np.pi * unfreq[0] * x)  # this is the original gabor filter

    filt_rows, filt_cols = reffilter.shape

    angleFin = int(180 / angleInc)

    gabor_filter = np.array(np.zeros((angleFin, filt_rows, filt_cols)))

    for o in range(0, angleFin):
